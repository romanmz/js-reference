...
var myVariable;					// local variable, global if defined outside a function. Leaks out of blocks
let myBlockScopedVariable;		// local variable, global if defined outside a function. Stays only inside their blocks
...
// const myConstant; // ERROR
const myConstant = 5;
// myConstant = 10; // ERROR
function myFunc() {}
// const myVariable = 10	// ERROR
// const myFunc = 5			// ERROR

// When assigning objects to constants, only the reference to the object itself is a constant, the individual properties may stil be changed


// trying to access an undefined variable will throw a reference error
// if(undefinedVar == 1 ) {} // ERROR
...
// VARIABLE 'HOISTING'
// 'var'iables declarations are lifted to the top of the function or statement, so you can reference them without getting an error, however their value will be 'undefined' until the statement that assigns an actual value to them
// this does not apply to 'let' variables
// functions are hoisted and execute correctly 'function foo()', but not function expressions 'var baz = function()'

// GLOBAL variables
// are properties of the globa object, 'window' in case of web pages
...
// variables can be converted from one type to another dynamically
// number become strings when added to a string with +, or ignored when using other operators

// to convert strings to numbers use:
// parseFloat() 	// always include the 'radix' as 2nd argument, usually 10 for decimal numbers
// parseInt()
// unary operator	// e.g. (+string1) + (+string2) == number

// array functions:
/*
Array.from( array-like )
Array,from( array-like, function( value, key ){ return value }, thisObject )
Array.of( value1, value2, etc... )
*/
...
var octalInteger = 0o10;				// begin with '0o', equals 8
var hexInteger = 0x10;					// begin with '0x', equals 16
var binaryInteger = 0b10;				// begin with '0b', equals 2
var floatNumber = 10.5;
var exponentNumber = 10.5e4;			// 10.5 x 10000 = 105000
var objectLiteral = {propertyName: 'propertyValue'}		// property names that are not valid js identifiers need to be defined within quotes, and can only be accessed with brackets syntax obj['!']
var extendedObjectLiteral = {
	...
	'func name'() { return 4 },			// shorthand for 'func name': function(){}
	['prop_'+decimalInteger]: 10,		// allows dynamic property names
	['method_'+decimalInteger](){},		// also works with methods
	get x() { return 1 },				// getter for property 'x'
	set x(v) { this.x = v },			// setter for property 'x'
	get ['prop2_'+decimalInteger](){},	// get and set can also be dynamic
	
	*generator(){}						// shorthand for 'generator: function*(){}'.  Also string-keyed 'generator name', and also computed [varname+'gen']. and also in class definitions
}
...
console.log( "lorem ipsum".length )	// you can call String properties and methods directly from a string literal

// Only `` can be used to create multi-line comments
var stringLiteralMultiline = `In javascript
is a line-feed`;

// Only `` can be used for string interpolation with ${}
...
// var a = { toString: function(){ return 'lol' } };
// `hello ${a}`			// returns 'hello lol'

// you can pass a template literal to a function that can deconstruct them
function deconstructLiteral( plainTextFragments, var1, var2 ) {
	console.log( 'deconstructed literal:' );
	console.log( plainTextFragments, plainTextFragments.raw, var1, var2 )
	// both plainTextFragments and plainTextFragments.raw are 'frozen' objects
}
